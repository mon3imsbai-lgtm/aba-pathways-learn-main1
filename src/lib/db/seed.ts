...
export async function seedDatabase() {
  ...
  // Seed site settings
  const { error: settingsError } = await supa.from("site_settings").upsert([
  ...
  // Seed features
  const { error: featuresError } = await supa.from("features").upsert([
  ...
  // Seed products
  const { data: products, error: productsError } = await supa.from("products").upsert([
  ...
  if (products && categories) {
    const { error: pcError } = await supa.from("product_categories").upsert([
  ...
  // Seed courses
  const { data: courses, error: coursesError } = await supa.from("courses").upsert([
  ...
  // Seed services
  const { error: servicesError } = await supa.from("services").upsert([
  ...
  // Seed blog posts
  const { error: blogError } = await supa.from("blog_posts").upsert([
  ...
  // Seed testimonials
  const { error: testimonialsError } = await supa.from("testimonials").upsert([
  ...
  // Seed FAQ items
  const { error: faqError } = await supa.from("faq_items").upsert([
